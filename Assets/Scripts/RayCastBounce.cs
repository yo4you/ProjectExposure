...
public class RayCastBounce : MonoBehaviour
{
	...
	private void Start()
	{
		...
		rippleBackgroundQuad.transform.position = pos;
	...
	private void Update()
	{
		...
		else if (Input.GetMouseButtonDown(0) && !_mousetracker.RayCastHitPlayer())
		{
			...
			if (Physics.Raycast(castPoint, out hit, Mathf.Infinity, _layerMask))
			{
				...
				if (_currentFlyingWaveIndex > _wavePoolCap - 1)
				{
					_currentFlyingWaveIndex = 0;
				}
				...
				for (int i = 0; i < _trails.Count; i++)
				{
					...
					if (!_trailsIdle[i][_currentFlyingWaveIndex])
					{
						if (_throwCoroutines[i][_currentFlyingWaveIndex] != null)
						{
	...
	private void CheckTrailsInactivity(int trailIndex, int poolIndex)
	{
		...
		{
			if (_throwCoroutines[trailIndex][poolIndex] != null)
			{
				StopCoroutine(_throwCoroutines[trailIndex][poolIndex]);
			}
	...
	{
		if (_trails[rayIndex][trailPoolIndex] == null || _trailsIdle[rayIndex][trailPoolIndex])
		{
			return;
		}
	...
	private IEnumerator ResetTrailDist(TrailRenderer trailRenderer)
	{
