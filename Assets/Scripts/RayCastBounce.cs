...
public class RayCastBounce : MonoBehaviour
{
    ...
    {
        // scale
        rippleBackgroundQuad.transform.localScale = FindObjectOfType<LevelMaterialFixer>().Scale;
        ...
        else if (Input.GetMouseButtonDown(0) && !_mousetracker.RayCastHitPlayer())
        {
            ...
            if (Physics.Raycast(castPoint, out hit, Mathf.Infinity, _layerMask))
            {
                ...
                for (int i = 0; i < _trails.Count; i++)
                {
                    ...
                    if (!_trailsIdle[i][_currentFlyingWaveIndex])
                    {
                        if (_throwCoroutines[i][_currentFlyingWaveIndex]!=null) StopCoroutine(_throwCoroutines[i][_currentFlyingWaveIndex]);
                        //_trailsPoints[i][_currentFlyingWaveIndex].Clear();
                        print("force stop");
